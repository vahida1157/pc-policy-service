...
class GlobalChildSettingsController(
	...
	fun getSettings(@PathVariable childId: UUID): ResponseEntity<GlobalSettingsResponse> {
		...
		val response = GlobalSettingsResponse(
			...
			// 🚀 MAP NEW FIELDS TO RESPONSE
			isExerciseRewardEnabled = settings.isExerciseRewardEnabled,
			rewardSecondsPerPoint = settings.rewardSecondsPerPoint,
			maxRewardSecondsPerDay = settings.maxRewardSecondsPerDay,
	...
	fun updateSettings(
		...
		if (settings.updatedAt == null || incomingTime.isAfter(settings.updatedAt)) {
			...
			settings.isExerciseRewardEnabled = request.isExerciseRewardEnabled
			settings.rewardSecondsPerPoint = request.rewardSecondsPerPoint
			settings.maxRewardSecondsPerDay = request.maxRewardSecondsPerDay
